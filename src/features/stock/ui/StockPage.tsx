  ...
  <div className="block sm:hidden">
    ...
    <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">SKU: {i.code}</div>
                  </div>
                  <span className={cx("px-2 py-0.5 text-xs font-semibold rounded-2xl", statusChip[i.status!])}>{
                    i.status === "in-stock" ? "En stock" : i.status === "low" ? "Bajo" : "Agotado"
                  }</span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="bg-slate-50 rounded-lg p-2">
                    <div className="flex items-center gap-1 text-xs text-slate-500">Cantidad<InfoButton text="Cantidad de producto disponible en bodega." /></div>
                    <div className="font-semibold">{i.stock}</div>
                  </div>
                  <div className="bg-slate-50 rounded-lg p-2">
                    <div className="flex items-center gap-1 text-xs text-slate-500">Pack<InfoButton text="Tamaño de pack o presentación (ej: 6, 12, 24, 36)." /></div>
                    <div className="font-semibold">{i.packSize}</div>
                  </div>
                  <div className="bg-slate-50 rounded-lg p-2">
                    <div className="flex items-center gap-1 text-xs text-slate-500">Precio<InfoButton text="Precio unitario de venta." /></div>
                    <div className="font-semibold">{formatCLP(i.price)}</div>
                  </div>
                  <div className="bg-slate-50 rounded-lg p-2">
                    <div className="flex items-center gap-1 text-xs text-slate-500">Costo<InfoButton text="Costo unitario de compra del producto." /></div>
                    <div className="font-semibold">{formatCLP(i.cost)}</div>
                  </div>
                  <div className="bg-slate-50 rounded-lg p-2 col-span-2">
                    <div className="flex items-center gap-1 text-xs text-slate-500">Valorización<InfoButton text="Valor total del stock actual según costo." /></div>
                    <div className="font-semibold">{formatCLP(i.stock * i.cost)}</div>
                  </div>
                </div>
              </div>
            ))}
            {items.length === 0 && <div className="p-6 text-center text-slate-500 text-sm">Sin resultados</div>}
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-100 text-center">
                ...
                <th className="p-3 text-center">Cantidad <InfoButton text="Cantidad de producto disponible en bodega." /></th>
                <th className="p-3 text-center">Pack <InfoButton text="Tamaño de pack o presentación (ej: 6, 12, 24, 36)." /></th>
                <th className="p-3 text-center">Costo <InfoButton text="Costo unitario de compra del producto." /></th>
                <th className="p-3 text-center">Precio <InfoButton text="Precio unitario de venta." /></th>
                <th className="p-3 text-center">Valorización <InfoButton text="Valor total del stock actual según costo." /></th>
                <th className="p-3 text-center">Ingreso potencial <InfoButton text="Ingreso total si se vende todo el stock al precio actual." /></th>
                <th className="p-3 text-center">Actualizado</th>
              </tr>
            </thead>
            <tbody>
              {items.map(i => (
                <tr key={i.id} className="border-t hover:bg-slate-50 text-center">
                  <td className="p-3 font-medium text-center">{i.name}</td>
                  <td className="p-3 text-center">{i.code}</td>
                  <td className="p-3 text-center"><span className={cx("px-2 py-0.5 text-xs font-semibold rounded-2xl", statusChip[i.status!])}>{ i.status === "in-stock" ? "En stock" : i.status === "low" ? "Bajo" : "Agotado" }</span></td>
                  <td className="p-3 text-center">{i.stock}</td>
                  <td className="p-3 text-center">{i.packSize}</td>
                  <td className="p-3 text-center">{formatCLP(i.cost)}</td>
                  <td className="p-3 text-center">{formatCLP(i.price)}</td>
                  <td className="p-3 text-center">{formatCLP(i.stock * i.cost)}</td>
                  <td className="p-3 text-center">{formatCLP(i.stock * i.price)}</td>
                  <td className="p-3 text-center">{new Date(i.updatedAt).toLocaleDateString("es-CL")}</td>
                </tr>
              ))}
              {items.length === 0 && (
                <tr><td className="p-6 text-center text-slate-500 text-sm" colSpan={10}>Sin resultados</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function KPI({ label, value, tone }: { label: string; value: string; tone: "slate"|"emerald"|"amber" }) {
  ...
  return (
    <div className={cx("rounded-xl border border-gray-100 p-3 sm:p-4", tones[tone])}>
...
function InfoButton({ text }: { text: string }) {
  return (
    <span className="relative group inline-block align-middle">
      <button className="ml-1 text-xs text-orange-500 hover:text-orange-700 focus:outline-none" tabIndex={0}>ⓘ</button>
      <div className="absolute left-1/2 -translate-x-1/2 mt-2 z-10 hidden group-hover:block group-focus:block min-w-[180px] bg-white border border-orange-200 rounded-lg shadow-lg p-2 text-xs text-slate-700">
        {text}
      </div>
    </span>
  );
