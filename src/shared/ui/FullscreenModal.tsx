...
export function FullscreenModal({ open, onClose, title, children }: { open: boolean; onClose: () => void; title?: ReactNode; children: ReactNode; }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 bg-black/40 animate-fadeIn" onClick={onClose}>
      <div className="fixed inset-0 bg-white animate-slideUp" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="sticky top-0 z-10 bg-white border-b border-gray-100 h-16 flex items-center justify-between px-4 sm:px-6">
