...
export default function SalesNotesPage() {
  ...
  const handleDownloadXLSX = async () => {
    if (!user || (!user.isAdmin && !user.isVendedor)) return;

    try {
      const response = await fetch(`/api/downloads/sales-notes?userId=${user.id}&isAdmin=${user.isAdmin}&isVendedor=${user.isVendedor}`);
      if (!response.ok) {
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between h-auto sm:h-16 py-3 sm:py-0 gap-3 sm:gap-0">
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto justify-end">
              ...
              <button
                ...
                disabled={!user || (!user.isAdmin && !user.isVendedor)}
